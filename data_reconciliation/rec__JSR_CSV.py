...
STATUS = {'Клеймо': 'Не соответствует клеймо',
          'Нет элемента': 'Не найден номер СС',
          'ДатаЗаключения': 'Не соответствуе дата заключения',
          'Материал': 'Не соответствует материал',
          'Дата работ': 'Дата работ позже НК',
          'Совпало': 'Полное совпадение',
          'Заключение': 'Отсутствует результат заключения',
          'Нет номера': 'Не найдено заключение НК',
        ...
        if row_jsr['Совп.Номер']:
            if row_jsr['Совп.Элемент']:
                ...
                if row_jsr['Клеймо'] != row_csv['Клеймо']:
                    ...
                    row_jsr['Акт проверки'].append(
                        f"{row_jsr['Свар.Элемент']} клеймо, указанное в {row_jsr['Тип']} №{row_jsr['Номер заключения']}, не соответствует клейму в ЖСР.")
                ...
                if row_jsr['Результат заключения'] != row_csv['Заключение']:
                    ...
                    row_jsr['Акт проверки'].append(
                        f"{row_jsr['Свар.Элемент']} отсутствует результат заключения в ЖСР.")
                ...
                if coll.checks_dates(row_jsr.setdefault('Дата работ', ''),
                    ...
                    row_jsr['Акт проверки'].append(
                        f"{row_jsr['Свар.Элемент']} дата {row_jsr['Тип']} №{row_jsr['Номер заключения']} раньше, чем дата выполнения работ в ЖСР.")
