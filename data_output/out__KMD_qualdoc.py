...
HEADERS = {'Марка': 'Марка из КМД',
           ...
           'Качества Номер из АООК': 'Упоминание в АООК',
           'АООК Дата': 'Дата АООК'}
...
def data_output(table, file_name):
    ...
    uf = {
        ...
        f'AND(ROW()>1, SEARCH("{STATUS["АООК != дата"]}",{col(HEADERS, "Качества Номер из АООК")}))': [
            {'Цвет': COLORS['Жёлтый'], 'Диапазон': col(HEADERS, ["Качества Номер из АООК", "Качества Номер из АООК"])}
        ],
        f'AND(ROW()>1, SEARCH("{STATUS["АООК ="]}",{col(HEADERS, "Качества Номер из АООК")}))': [
            {'Цвет': COLORS['Зелёный'], 'Диапазон': col(HEADERS, ["Качества Номер из АООК", "Качества Номер из АООК"])}
        ],
        f'AND(ROW()>1, SEARCH("{STATUS["АООК !="]}",{col(HEADERS, "Качества Номер из АООК")}))': [
            {'Цвет': COLORS['Красный светлый'], 'Диапазон': col(HEADERS, ["Качества Номер из АООК",
                                                                               "Качества Номер из АООК"])}
        ],
        f'AND(ROW()>1, SEARCH("{STATUS["АООК < ДК"]}",{col(HEADERS, "АООК Дата")}))': [
