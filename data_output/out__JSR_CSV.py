...
def data_output(table, file_name):
    if not table:
        print('!!! Сврека ЖСР с НК отсутствует, лист не будет создан')
        return None
    ...
    uf = {
        ...
        f'AND(ROW()>1, SEARCH("{STATUS["Нет элемента"]}",{col(HEADERS, "Статус проверки")}))': [
            {'Цвет': COLORS['Красный светлый'], 'Диапазон': col(HEADERS, ["Свар.Элемент", "Свар.Элемент"])},
            {'Цвет': COLORS['Зелёный светлый'], 'Диапазон': col(HEADERS, ["Тип", "Номер заключения"])}
        ],
