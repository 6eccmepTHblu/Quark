...
from data_reconciliation.rec__quardoc_AoRPI import STATUS
from consts.colors import COLORS

SHEET_NAME = "Качество_АоРПИ"
...
def data_output(table, file_name):
    ...
    styles.SheetStyle(sh, 55, [1, 2], 80, COLORS['Синий светлый'])
    ...
    uf = {
        ...
        f'AND(ROW()>1, SEARCH("{STATUS["Марка"]}",{col(HEADERS, "Статус проверки")}))': [
            {'Цвет': COLORS['Красный светлый'], 'Диапазон': col(HEADERS, ["Номер АоРПИ", "Количество"])}
        ],
        f'AND(ROW()>1, SEARCH("{STATUS["Дата из АоРПИ"]}",{col(HEADERS, "Статус проверки")}))': [
            {'Цвет': COLORS['Жёлтый'], 'Диапазон': col(HEADERS, ["Дата отгрузки", "Дата отгрузки"])}
        ],
        f'AND(ROW()>1, SEARCH("{STATUS["Дата ДК и АоРПИ"]}",{col(HEADERS, "Статус проверки")}))': [
            {'Цвет': COLORS['Жёлтый'], 'Диапазон': col(HEADERS, ["Дата отгрузки", "Дата отгрузки"])},
            {'Цвет': COLORS['Жёлтый'], 'Диапазон': col(HEADERS, ["Дата АоРПИ", "Дата АоРПИ"])}
        ],
        f'AND(ROW()>1, SEARCH("{STATUS["АоРПИ номер в ДК"]}",{col(HEADERS, "Статус проверки")}))': [
            {'Цвет': COLORS['Жёлтый'], 'Диапазон': col(HEADERS, ["Номер документа", "Номер документа"])},
        ]
    }
    uf_color(uf, sh)
